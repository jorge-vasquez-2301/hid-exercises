...
spec_buildIP =
  describe "buildIP" $ do
    ...
    it "builds from arbitrary address" $
      buildIP [192, 168, 3, 15]
        `shouldBe` (IP $ 15 + 3 * 256 + 168 * 256 ^ 2 + 192 * 256 ^ 3)
  ...
  describe "parseIP" $ do
    ...
    it "parses some random IP address" $
      parseIP "192.168.3.15"
        `shouldBe` Just (buildIP [192, 168, 3, 15])
...
spec_parseIPRange =
  describe "parseIPRange" $ do
    it "parses pair" $
      parseIPRange "192.168.0.1,192.168.3.100"
        `shouldBe` IPRange <$> parseIP "192.168.0.1" <*> parseIP "192.168.3.100"
  ...
  describe "parseIPRanges" $ do
    ...
    it "parses single range" $
      parseIPRanges sample_range
        `shouldBe` Right (IPRangeDB $ catMaybes [parseIPRange sample_range])
    it "parses two ranges" $
      parseIPRanges (sample_range ++ "\n" ++ sample_range2)
        `shouldBe` Right
          ( IPRangeDB $
              catMaybes
                [ parseIPRange sample_range,
